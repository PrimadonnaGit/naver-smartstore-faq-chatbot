.PHONY: install setup run pre-start redis-up redis-down help

help:
	@echo "사용 가능한 명령어:"
	@echo "make install      - Poetry 및 필요한 패키지 설치"
	@echo "make setup        - 초기 환경 설정 (디렉토리 생성 등)"
	@echo "make pre-start   - FAQ 데이터 전처리 실행 및 Embedding 적재"
...
pre-start:
	PYTHONPATH=src poetry run python scripts/prestart.py
